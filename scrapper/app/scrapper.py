...
def pages(page: str) -> dict[Any]:
    ...
    html = Soup(request.content, features='html.parser')
    ...
    for (index, button) in enumerate(buttons):
        ...
        if button.has_attr('data-id'):
            (_, category) = button['data-id'].strip('/').split('/', 1)
            # print(_page, '::', _category)
            row['name'] = category
...
def feeds(page: str, category: str) -> dict[Any]:
    request = requests.get(f'{URL}/{page}/{category}')
    html = Soup(request.content, features='html.parser')
    # get all divs with data-testid = "MediaStoryCard"
    # cards = html.find_all('div', {'data-testid': 'MediaStoryCard'})
    cards = html.find_all('li', id=True)
    data = []
    for (index, card) in enumerate(cards):
        # print(button.text)
        row = {'id': None, 'category': None, 'title': None,
    ...
    return {'total': len(data), 'source': URL, 'page': page, 'category': category, 'items': data}
